...
```{r Create Training and Validation Test Sets}
set.seed(4896)
training_set<-createDataPartition(training_cleaned$classe, p=0.75, list=FALSE)
train_set<-training_cleaned[training_set,]
validate_set<-training_cleaned[-training_set,]
```
# Model Fitting
Here, we will use the Random Forest model for classification. 
```{r Model Fitting, message=FALSE, results='hide', warning=FALSE}
rfFit<-train(classe~., data=train_set, method="rf")
```
## Training set Accuracy
```{r Training set Accuracy}
rfPred<-predict(rfFit, train_set)
rfMat<-confusionMatrix(rfPred, train_set$classe)
rfMat$overall
```
